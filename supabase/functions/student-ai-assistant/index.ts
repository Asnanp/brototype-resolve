...
serve(async (req) => {
  ...
  try {
    ...
    if (!checkRateLimit(userId)) {
      return new Response(JSON.stringify({ error: 'Rate limit exceeded. Please wait a moment before asking more questions.' }), {
        status: 429,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
...
Never make promises about resolution times or outcomes - only explain the process.`;
    ...
    console.error('Error in student-ai-assistant:', error);
